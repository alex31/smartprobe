...
/*
  template class to use a variable of type T as a whiteboard :

  the whiteboard include an event source, and a RW lock protection.
...
enum class UpdateBehavior {Verify, Always}; 
...
void WhiteBoard<T, ub>::write(const T &c) {
  writeLock();
  if constexpr (ub == UpdateBehavior::Verify) {
...
template<typename T, UpdateBehavior ub>
void WhiteBoard<T, ub>::read(T &c, const eventmask_t events) const {
  chEvtWaitOne(events);
  read(c);
} ;

template<typename T, UpdateBehavior ub>
void WhiteBoard<T, ub>::read(T &c) const {
  readLock();
  c = v;
  readUnlock();
} ;
...
template<typename T, UpdateBehavior ub>
void WhiteBoard<T, ub>::readLock(void) const
{
  chMtxLock(&mu);
...
template<typename T, UpdateBehavior ub>
void WhiteBoard<T, ub>::readUnlock(void) const
{
  chMtxLock(&mu);
  if (--readersWaiting == 0) {
...
template<typename T, UpdateBehavior ub>
void WhiteBoard<T, ub>::writeLock(void) const
{
  chMtxLock(&mu);
  while (writerWaiting == true) {
    chCondWait(&cond);
...
template<typename T, UpdateBehavior ub>
void WhiteBoard<T, ub>::writeUnlock(void) const
{
  chMtxLock(&mu);
  writerWaiting = false;
  chCondBroadcast(&cond);
...
template<typename T, UpdateBehavior ub>
void WhiteBoard<T, ub>::broadcastUpdate(void) const
{
...
template<typename T, UpdateBehavior ub>
void WhiteBoard<T, ub>::registerEvt(event_listener_t *lst, const eventmask_t events) const
{
